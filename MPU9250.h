...
class MPU9250
{
    ...
    int ak_i2c_addr{AK8963_ADDRESS};
...
public:
    ...
    bool setup(const uint8_t addr, const MPU9250Setting &mpu_setting = MPU9250Setting())
    {
        ...
        mpu_i2c_addr = wiringPiI2CSetup(addr);
        setting = mpu_setting;
        i2c_fd = mpu_i2c_addr;
    ...
    {
        byte c = read_byte(ak_i2c_addr, AK8963_WHO_AM_I);
        if (b_verbose)
        {
            printf("AK8963 WHO AM I = 0x");
            printf("%x\n", c);
        }
...
private:
    void initMPU9250()
    {
        ...
        write_byte(mpu_i2c_addr, INT_PIN_CFG, 0x02);
        //write_byte(mpu_i2c_addr, INT_ENABLE, 0x01); // Enable data ready (bit 0) interrupt
        ak_i2c_addr = wiringPiI2CSetup(AK8963_ADDRESS);
        delay(100);
    }
    ...
    void initAK8963()
    {
        ...
        write_byte(ak_i2c_addr, AK8963_CNTL, 0x00); // Power down magnetometer
        delay(10);
        write_byte(ak_i2c_addr, AK8963_CNTL, 0x0F); // Enter Fuse ROM access mode
        delay(10);
        read_bytes(ak_i2c_addr, AK8963_ASAX, 3, &raw_data[0]);     // Read the x-, y-, and z-axis calibration values
        ...
        write_byte(ak_i2c_addr, AK8963_CNTL, (uint8_t)setting.mag_output_bits << 4 | MAG_MODE); // Set magnetometer data resolution and sample ODR
        delay(10);
...
private:
    ...
    {
        const uint8_t st1 = read_byte(ak_i2c_addr, AK8963_ST1);
        if (st1 & 0x01)
        {                                                               // wait for magnetometer data ready bit to be set
            uint8_t raw_data[7];                                        // x/y/z gyro register data, ST2 register stored here, must read ST2 at end of data acquisition
            read_bytes(ak_i2c_addr, AK8963_XOUT_L, 7, &raw_data[0]); // Read the six raw data and ST2 registers sequentially into data array
    ...
    void calibrate_mag_impl()
    {
        ...
        if (b_verbose)
        {
            ...
            printf("%f, %f, %f\nAK8963 mag scale (mG)\n%f, %f, %f\n", mag_bias[0], mag_bias[1], mag_bias[2], mag_scale[0], mag_scale[1], mag_scale[2]);
        }
    ...
    {
        i2c_err_ = wiringPiI2CWriteReg8(address, subAddress, data);
        if (i2c_err_)
            print_i2c_error();
    }
    ...
    {
        uint8_t data = wiringPiI2CReadReg8(address, subAddress); // `data` will store the register data
    ...
    void read_bytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t *dest)
    {
        ...
        {
            dest[k] = wiringPiI2CReadReg8(address, subAddress + k);
        }
    }

    void print_i2c_error()
    {
        printf("I2C ERROR CODE : %d", i2c_err_);
    }
};
